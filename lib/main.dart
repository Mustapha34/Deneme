...
class PersonDB{
  ...
  StreamController<List<Person>> _streamController = StreamController<List<Person>>.broadcast();
  ...
  Stream<List<Person>> all()=> _streamController.stream.map((persons) => persons..sort());
...
class ComposeWidget extends StatefulWidget {
  ...
  ComposeWidget({
    Key? key, required this.onCompose}) : 
    super(key : key);
  @override
